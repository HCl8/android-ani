...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        // 1TODO ex2-1：在这里实现另一个 ObjectAnimator，对 target 控件的大小进行缩放，从 1 到 2 循环

        // 1TODO ex2-2：在这里实现另一个 ObjectAnimator，对 target 控件的透明度进行修改，从 1 到 0.5f 循环

        // 1TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        ...
        ObjectAnimator fadeAnimator = ObjectAnimator.ofFloat(target,
                "alpha", 1f, 0.5f);
        fadeAnimator.setRepeatCount(ObjectAnimator.INFINITE);
        fadeAnimator.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        fadeAnimator.setRepeatMode(ObjectAnimator.REVERSE);

        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1,scaleXAnimator,scaleYAnimator,fadeAnimator);
        animatorSet.start();
    }
}
